...
function PrintableInvoice({ invoice, type }: { invoice: Invoice; type: "achat" | "vente" }) {
  ...
  const docType = type === "achat"
    ? (invoice.statut === "valide" ? "BON DE RÉCEPTION" : "BON DE COMMANDE")
    : (invoice.statut === "valide" ? "BON DE LIVRAISON" : "FACTURE PROFORMA");
  ...
  const totals = [
    {
      label: "Sous-total",
      value: formatGNF(invoice.montant_total),
    },
    { label: "TOTAL", value: formatGNF(invoice.montant_total), strong: true },
    ...(totalPaid > 0
      ? [
          { label: "Déjà payé", value: formatGNF(totalPaid), muted: true },
          {
            label: restant > 0 ? "Reste à payer" : "Soldé",
            value: formatGNF(Math.max(restant, 0)),
            strong: restant > 0,
          },
        ]
      : []),
  ...
  return (
    <div className="hidden print:block">
      <PrintableDocument
        docType={docType}
        reference={invoice.reference ? invoice.reference : `#${invoice.id}`}
        date={dateDoc ? formatDateFr(dateDoc) : undefined}
        secondaryDate={
          dateValid
            ? {
                label: type === "achat" ? "Date de réception" : "Date de livraison",
                value: formatDateFr(dateValid),
              }
            : undefined
        }
        status={invoice.statut?.toUpperCase()}
        organisation={buildOrganisation(user ?? undefined)}
        party={{
          label: type === "achat" ? "Fournisseur" : "Client",
          name: partner,
        }}
        meta={[
          { label: "Référence", value: invoice.reference || `#${invoice.id}` },
          { label: "Statut document", value: invoice.statut || "—" },
          { label: "Statut paiement", value: invoice.statut_paiement || "—" },
          { label: "Articles", value: String(invoice.lignes?.length ?? 0) },
        ]}
        lines={invoice.lignes.map((l) => ({
          description: l.produit_nom,
          quantite: l.quantite,
          prixUnitaire: l.prix_unitaire,
          total: l.sous_total,
        }))}
        totals={totals}
        notes={invoice.observations}
        signatures={[
          { label: type === "achat" ? "Signature fournisseur" : "Signature client" },
